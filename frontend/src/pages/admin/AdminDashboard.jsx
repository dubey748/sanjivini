...
import {
  Users, ShoppingBag, Pill, IndianRupee, TrendingUp, Store, Database, ShieldCheck,
} from "lucide-react";
import {
...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      .then(([s, o, u, h]) => {
        if (cancelled) return;
        setStats(s); setOrders(o); setUsers(u); setHealth(h);
      })
  ...
  if (!stats) {
    return (
      <div className="py-10 text-center text-muted-foreground" data-testid="admin-dashboard-loading">
        Loading dashboard…
      </div>
    );
  }
  ...
  return (
    <div className="mx-auto max-w-6xl" data-testid="admin-dashboard">
      <div className="flex flex-col items-start justify-between gap-3 md:flex-row md:items-center">
        <div>
          ...
          <p className="text-muted-foreground">
            Sanjeevni · Real-time operations overview
          </p>
        </div>
        {health && (
          <Card className="rounded-2xl border-[#0F4C3A]/20">
            <CardContent className="flex items-center gap-3 p-3 text-xs">
              <ShieldCheck className="h-4 w-4 text-[#0F4C3A]" />
              <div>
                <div className="font-semibold text-[#0F4C3A]">
                  DB {health.db_ok ? "online" : "offline"} · {health.collections_count} collections
                </div>
                <div className="text-muted-foreground">
                  Last migration: {health.last_migration?.id || "none"}
                </div>
              </div>
            </CardContent>
          </Card>
      ...
      <div className="mt-8 grid gap-6 lg:grid-cols-3">
        <Card className="rounded-3xl lg:col-span-2">
          <CardContent className="p-6">
            <h3 className="font-display flex items-center gap-2 text-lg font-semibold">
              <TrendingUp className="h-4 w-4 text-[#0F4C3A]" /> Revenue (last 7 days)
            </h3>
        ...
        <Card className="rounded-3xl">
          <CardContent className="p-6">
            <h3 className="font-display flex items-center gap-2 text-lg font-semibold">
              <Store className="h-4 w-4 text-[#0F4C3A]" /> Order status
            </h3>
            <div className="mt-4 h-64 min-h-[256px] w-full">
              <ResponsiveContainer width="100%" height="100%">
                ...
                {orders.map((o) => (
                  <tr key={o.id} className="border-t border-border" data-testid={`admin-order-${o.id}`}>
                    ...
                    <td className="px-4 py-3">
                      <Badge className="rounded-full bg-[#2D7A5D]/10 text-[#2D7A5D] hover:bg-[#2D7A5D]/10">
                        {o.status}
                      </Badge>
                    </td>
